  ...
  const getAllHandloansWithBalances = () => {
    return handloans.map((h) => {
      ...
      const balance = Number(h.balanceAmount) || 0;
      const recovered = loanAmount - balance;

      let status = "ISSUED";
      if (balance === 0 && loanAmount > 0) status = "RECOVERED";
      else if (recovered > 0 && balance > 0) status = "PARTIALLY RECOVERED";
                      ...
                      <tbody>
                        {allHandloansWithBalances.map((loan, idx) => (
                          <tr key={idx} style={styles.tableRow}>
                            ...
                            <td style={styles.tableCell}>
                              {safeToLocaleString(
                                loan.loanAmount - loan.recoveredAmount
                              )}
                            </td>
                            <td
                              style={{
